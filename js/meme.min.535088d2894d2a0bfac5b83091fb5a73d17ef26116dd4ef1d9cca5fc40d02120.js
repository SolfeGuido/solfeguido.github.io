;const userPrefers=getComputedStyle(document.documentElement).getPropertyValue('content');if(theme==="dark"){goDark();}else if(theme==="light"){goLight();}else if(userPrefers==="dark"){goDarkMeta();setDark();goDark();}else if(userPrefers==="light"){goLightMeta();setLight();goLight();}
function modeSwitcher(){const currentMode=document.documentElement.getAttribute('data-theme');if(currentMode==="dark"){goLightMeta();setLight();goLight();}else{goDarkMeta();setDark();goDark();}}
const darkModeMediaQuery=window.matchMedia('(prefers-color-scheme: dark)');darkModeMediaQuery.addListener((e)=>{const darkModeOn=e.matches;if(darkModeOn){goDarkMeta();setDark();goDark();}});const lightModeMediaQuery=window.matchMedia('(prefers-color-scheme: light)');lightModeMediaQuery.addListener((e)=>{const lightModeOn=e.matches;if(lightModeOn){goLightMeta();setLight();goLight();}});window.addEventListener('storage',function(event){if(event.newValue==="dark"){goDarkMeta();goDark();}else{goLightMeta();goLight();}});function goDarkMeta(){document.documentElement.setAttribute('data-theme','dark');document.querySelector('meta[name="theme-color"]').setAttribute('content','#1c1c21');}
function goLightMeta(){document.documentElement.setAttribute('data-theme','light');document.querySelector('meta[name="theme-color"]').setAttribute('content','#fff');}
function setDark(){window.localStorage.setItem('theme','dark');}
function setLight(){window.localStorage.setItem('theme','light');}
function goDark(){document.getElementById("theme-toggle").innerHTML="🌙";var els=[].slice.apply(document.getElementsByClassName("chroma"));for(var i=0;i<els.length;i++){els[i].className=els[i].className.replace(/ *\bchroma\b/g,"chroma-dark");}}
function goLight(){document.getElementById("theme-toggle").innerHTML="🌞";var els=[].slice.apply(document.getElementsByClassName("chroma-dark"));for(var i=0;i<els.length;i++){els[i].className=els[i].className.replace(/ *\bchroma-dark\b/g,"chroma");}};var InstantClick=function(document,location){var $ua=navigator.userAgent,$isChromeForIOS=$ua.indexOf(' CriOS/')>-1,$hasTouch='createTouch'in document,$currentLocationWithoutHash,$urlToPreload,$preloadTimer,$lastTouchTimestamp,$history={},$xhr,$url=false,$title=false,$mustRedirect=false,$body=false,$timing={},$isPreloading=false,$isWaitingForCompletion=false,$trackedAssets=[],$useWhitelist,$preloadOnMousedown,$delayBeforePreload,$eventsCallbacks={fetch:[],receive:[],wait:[],change:[]}
